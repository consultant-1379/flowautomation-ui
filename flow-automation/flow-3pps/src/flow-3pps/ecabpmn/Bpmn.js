define(['jscore/base/jquery', 'flow-3pps/ecabpmn/Transformer', 'flow-3pps/ecabpmn/Renderer'], function ($, Transformer, Renderer) {
  ...
  Bpmn.prototype.zoom = function (zoomFactor) {
    var factor = this.scale*zoomFactor;
  ...
  Bpmn.prototype.getOverlays = function() {
    ...
    return document.getElementsByClassName(this.xid+"bpmnElement");
  };

  Bpmn.prototype.getOverlay = function(id) {
    // return $("#" + id);
    return document.getElementsByClassName(this.xid+id)[0];
  };
  ...
  Bpmn.prototype.annotation = function (id) {
    ...
    return {
      ...
      /**
       * Removes divs attached to the annotation
       */
      removeDivs : function() {
      ...
      removeClassesByPrefix : function (prefix) {
          // element.removeClass((classesArray || []).join(" "));
          var currentClasses = element.className.split(/ +/);
          var newClasses = [];
          for (var i = 0; i < currentClasses.length; i++) {
            if (!currentClasses[i].startsWith(prefix)) {
              newClasses.push(currentClasses[i]);
            }
          }
          var newClassName = "";
