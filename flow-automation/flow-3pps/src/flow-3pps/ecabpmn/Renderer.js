...
define([ "flow-3pps/dojo/dojox/gfx", "jscore/base/jquery" ], function (gfx, $) {
  ...
  function renderLabel(elementRenderer, group, bounds, align, moveUp) {
    ...
    var pos = labelBounds ? {x: +labelBounds.x + BpmnElementRenderer.labelPadding, y: +labelBounds.y + BpmnElementRenderer.labelPadding+10} : {x: +bounds.x, y: +bounds.y};
  ...
  var activityRenderer = {
    render : function(elementRenderer, gfxGroup) {
      ...
      renderLabel(elementRenderer, gfxGroup, {x: x + width /2 , y: y + 6 + height /2}, "middle", true);
    }
  };
  ...
  BpmnElementRenderer.prototype.render = function(options, gfxGroup) {
    ...
    var xid = (options.xid && options.xid != null) ? options.xid : "";
    ...
    for (var index in elements) {
      ...
      if (bounds && !options.skipOverlays) {
        ...
        var diagramElement = document.getElementById(options.diagramElement)
        diagramElement.style.position = "relative";

        var overlayDiv = $('<div class="' + xid+currentElement.id + ' ' + xid+'bpmnElement"></div>');
      ...
      if(!!currentElement.baseElements) {
        ...
        for(var i = 0; i < currentElement.baseElements.length; i++) {
          var bpmnElementRenderer = new BpmnElementRenderer(currentElement.baseElements[i], this);
          if (bpmnElementRenderer.baseElement.bpmndi != null) {
              bpmnElementRenderer.render(renderingOpts, gfxGroup);
          }
        }
